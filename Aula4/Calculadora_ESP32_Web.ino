/*
  ...
  Operacoes: soma, subtracao, multiplicacao e fatorial.
...
<body>
  <div class="card">
    ...
    <div class="subtitulo">Soma, subtração, multiplicação e fatorial — cálculo feito em C/C++ na ESP32</div>
...
void handleCalc() {
  ...
  if (op != "add" && op != "sub" && op != "mul" && op != "fact") { server.send(400, "application/json", buildErrorJson("Erro: operacao invalida.")); return; }
  ...
  String opName = op == "add" ? "SOMA" : op == "sub" ? "SUBTRACAO" : op == "mul" ? "MULTIPLICACAO" : "FATORIAL DE A";
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Iniciando Calculadora ESP32 Web - soma/sub/mul/fatorial");
